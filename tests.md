...
4. If the skill is shown in a skill list or chip UI, confirm the small icon uses `semantier-logo-small.svg`, the large artwork uses `semantier-logo.png`, and the skill is associated with `semantier.com`.
...
- The skill branding uses the Semantier logo when icon metadata is surfaced.

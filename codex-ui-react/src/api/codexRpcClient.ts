import type { RpcEnvelope, RpcNotification } from '../types/codex';
...
export class CodexApiError extends Error {
  ...
  constructor(message: string, { code, method, status }: { code: string; method?: string; status?: number }) {
    super(message);
    this.code = code;
    this.method = method;
    this.status = status;
...
function extractErrorMessage(payload: unknown, fallback: string): string {
  ...
  const nestedError = record.error as Record<string, string> | undefined;
...
export async function fetchRpcMethodCatalog(): Promise<string[]> {
  ...
  const catalog = payload as { data: string[] };
...
function createRpcNotificationSubscription(
  onNotification: (value: RpcNotification) => void
): () => void {
...
const sharedRpcListeners = new Set<(value: RpcNotification) => void>();
let sharedRpcCleanup: (() => void) | null = null;
let sharedRpcTeardownTimer: number | null = null;

function emitSharedRpcNotification(value: RpcNotification): void {
  for (const listener of sharedRpcListeners) {
    listener(value);
  }
}

function ensureSharedRpcSubscription(): void {
  if (typeof window === 'undefined') return;
  if (sharedRpcTeardownTimer !== null) {
    window.clearTimeout(sharedRpcTeardownTimer);
    sharedRpcTeardownTimer = null;
  }
  if (sharedRpcCleanup) return;
  sharedRpcCleanup = createRpcNotificationSubscription(emitSharedRpcNotification);
}

function scheduleSharedRpcTeardown(): void {
  if (typeof window === 'undefined') return;
  if (!sharedRpcCleanup || sharedRpcListeners.size > 0 || sharedRpcTeardownTimer !== null) return;
  sharedRpcTeardownTimer = window.setTimeout(() => {
    sharedRpcTeardownTimer = null;
    if (sharedRpcListeners.size > 0 || !sharedRpcCleanup) return;
    sharedRpcCleanup();
    sharedRpcCleanup = null;
  }, 1000);
}
...
export function subscribeRpcNotifications(
  ...
  sharedRpcListeners.add(onNotification);
  ensureSharedRpcSubscription();

  return () => {
    sharedRpcListeners.delete(onNotification);
    scheduleSharedRpcTeardown();
  };
}

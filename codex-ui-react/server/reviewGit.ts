...
export type ReviewActionLevel = 'all' | 'file' | 'hunk';
...
async function detectBaseBranch(repoRoot: string): Promise<DetectedBaseBranch | null> {
  const originHead = await runCommandResult(
    'git',
...
async function resolveBaseBranch(repoRoot: string, requestedBaseBranch = ''): Promise<DetectedBaseBranch | null> {
  ...
  if (normalizedRequested) {
    for (const candidate of [normalizedRequested, `origin/${normalizedRequested}`]) {
      if (await gitRefExists(repoRoot, candidate)) {
        return { displayName: normalizedRequested, gitRef: candidate };
      }
    }
  }
...
async function diffUntrackedFile(repoRoot: string, pathValue: string): Promise<string> {
  const result = await runCommandResult(
    'git',
    ['diff', '--no-index', '--no-ext-diff', '--patch', '--', '/dev/null', pathValue],
    { cwd: repoRoot },
  );
  if (result.code !== 0 && result.code !== 1) {
    const details = [result.stderr, result.stdout].filter(Boolean).join('\n');
    const suffix = details ? `: ${details}` : '';
    throw new Error(`Command failed (git diff --no-index -- /dev/null ${pathValue})${suffix}`);
  }
...
async function buildWorkspaceDiff(repoRoot: string, workspaceView: ReviewWorkspaceView): Promise<string> {
  if (workspaceView === 'staged') {
    try {
      return await runCommandCapture('git', ['diff', '--cached', '--no-ext-diff', '--find-renames', '--patch'], {
        cwd: repoRoot,
      });
    } catch (error) {
  ...
  let trackedDiff = '';
  try {
    trackedDiff = await runCommandCapture('git', ['diff', '--no-ext-diff', '--find-renames', '--patch'], {
      cwd: repoRoot,
    });
  } catch (error) {
    if (!isMissingHeadError(error)) {
  ...
  const statusOutput = await runCommandCapture('git', ['status', '--porcelain=v1', '--untracked-files=all'], {
    cwd: repoRoot,
  });
  const untrackedDiffs = await Promise.all(
    parseUntrackedPaths(statusOutput).map(async (pathValue) => await diffUntrackedFile(repoRoot, pathValue)),
  );
...
function buildReviewDiffLines(
  fileId: string,
  hunkId: string,
  lines: string[],
): { addedLineCount: number; removedLineCount: number; lines: ReviewDiffLine[] } {
...
function parseReviewSnapshotFile(repoRoot: string, blockLines: string[], fileIndex: number): ReviewSnapshotFile | null {
  ...
  return {
    ...
    operation,
    addedLineCount: hunks.reduce((sum, hunk) => sum + hunk.addedLineCount, 0),
    removedLineCount: hunks.reduce((sum, hunk) => sum + hunk.removedLineCount, 0),
...
async function writePatchFile(patch: string): Promise<string> {
  const dir = join(tmpdir(), 'codexui-review-patches');
...
export async function applyReviewAction(payload: unknown): Promise<ReviewSnapshot> {
  const record = asRecord(payload);
  if (!record) {
  ...
  const scope: ReviewScope = record.scope === 'baseBranch' ? 'baseBranch' : 'workspace';
  const workspaceView: ReviewWorkspaceView = record.workspaceView === 'staged' ? 'staged' : 'unstaged';
  ...
  return await getReviewSnapshot(normalizedCwd, scope, workspaceView);
}
